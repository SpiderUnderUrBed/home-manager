...
let
  ...
  waybarConfig = ((import ./hyprland/waybar.nix) {
    lib = lib;
    pkgs = pkgs;
  }) // {
    enable = true;
  };
  hyprlandConfig = ((import ./hyprland/hyprland.nix) {
    lib = lib;
    pkgs = pkgs;
    inputs = inputs;
  }) // {
  ...
  hyprlockConfig = ((import ./hyprland/hyprlock.nix) {
    lib = lib;
    pkgs = pkgs;
  }) // {
    enable = true;
...
in {
  ...
  programs = {
    ...
    wofi = { enable = wofiEnable; };
    rofi = { enable = lib.mkForce rofiEnable; };
    ...
    dunst = {
      ...
      settings = {
        global = {
          ...
          max_icon_size = 80;
          icon_path =
            "/usr/share/icons/Arc/status/96:/usr/share/icons/Arc/status/symbolic";
